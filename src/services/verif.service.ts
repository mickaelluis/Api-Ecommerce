...
const VeriftFobip =process.env.INFOBIP_API_KEY
const client = twilio(accountSid, authToken);
if (!verifySid) {
  // Verifica se o SID de verificação está definido
  throw new Error("TWILIO_VERIFY_SID não está definido no .env");
}

const verificacao = {
  // metodo para iniciar a verificação do telefone pelo TWILIO
  iniciarVerificacaoTelefoneTWILIO: async (clienteId: ObjectId) => {
    try {
      const filtro = { _id: clienteId };
      const resultadoDaBusca = await clientes.findOne(
        filtro,
        "telefone.Number -_id"
      );
      ...
      if (
        resultadoDaBusca &&
        resultadoDaBusca.telefone &&
        resultadoDaBusca.telefone.Number
      ) {
        ...
        throw new Error(
          "Não foi possível encontrar um número de telefone para este cliente."
        );
      }
      ...
      const verification = await client.verify.v2
        .services(verifySid)
        .verifications.create({ to: NumeroForaDoArray, channel: "sms" });
      ...
      const NumeroCensurrado =
        NumerosCensurrado +
        NumeroForaDoArray[10] +
        NumeroForaDoArray[11] +
        NumeroForaDoArray[12] +
        NumeroForaDoArray[13];
      // Retorna uma mensagem de sucesso, informando que o SMS foi enviado com sucesso
      return {
        success: true,
        message: `SMS de verificação enviado para o numero ${NumeroCensurrado}`,
      };
    } catch (error) {
  ...
  confirmarCodigoTelefoneTWILIO: async (
    clienteId: ObjectId,
    ...
    try {
      ...
      const verificationCheck = await client.verify.v2
        .services(verifySid)
        .verificationChecks.create({ to: NumeroForaDoArray, code: codigo });
      ...
      if (verificationCheck.status === "approved") {
        ...
        const autenticacao = {
          $set: { telefone: [{ Number: numeroTelefone, auth: "true" }] },
        };
